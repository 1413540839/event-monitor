# -*- coding: utf-8 -*-
"""Perpetual Futures Monitor v1 - 1H Mean Reversion + Multi-directional
...
P20_LOW = 0.30; P20_HIGH = 0.70
VR_MIN = 1.0
SL_PCT = 1.0; TP_PCT = 1.5
MAX_HOLD_BARS = 4
MARGIN = 10; LEVERAGE = 20
NOTIONAL = MARGIN * LEVERAGE  # 200u
...
def load_trades():
    ...
    return pd.DataFrame(columns=["time","coin","direction","entry_price","exit_price","pnl","result","detail"])
...
def fetch_candles(sym):
    ...
    try:
        ...
        if r.json()["code"] != "0":
            log.warning("API error %s: %s", sym, r.json().get("msg",""))
            return None
...
def compute_signal(df):
    """Returns (direction, score_dict) or (0, None)"""
    ...
    lp20 = p20.iloc[-2]; lvr = vr20.iloc[-2]
    
    if pd.isna(lr7) or pd.isna(lp20) or pd.isna(lvr):
        return 0, None
...
def check_exits(dfs_current):
    """Check if pending trades hit SL or TP"""
    results = []; to_remove = []
    for (sym, entry_ts), trade in list(PENDING.items()):
        ...
        if len(future_bars) == 0: continue
        
        exit_price = None; exit_reason = ""
        sl_price = entry * (1 - SL_PCT/100) if direction == 1 else entry * (1 + SL_PCT/100)
        tp_price = entry * (1 + TP_PCT/100) if direction == 1 else entry * (1 - TP_PCT/100)
        
        for idx in range(min(len(future_bars), MAX_HOLD_BARS)):
            bar = future_bars.iloc[idx]
            if direction == 1:
            ...
            if len(future_bars) >= MAX_HOLD_BARS:
                exit_price = future_bars.iloc[MAX_HOLD_BARS-1]["close"]
                exit_reason = "TIME"
            else:
        ...
        is_win = (direction == 1 and exit_price > entry) or (direction == -1 and exit_price < entry)
        pct_move = abs(exit_price - entry) / entry * 100 * direction
        pnl = pct_move * NOTIONAL / 100  # simplified
        
        # More accurate PnL: use actual exit price
        if direction == 1:
            actual_pnl = (exit_price - entry) / entry * NOTIONAL
        else:
            actual_pnl = (entry - exit_price) / entry * NOTIONAL
        
        result = "WIN" if actual_pnl > 0 else "LOSS"
        results.append({
            "time": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            "coin": trade["coin"], "direction": "LONG" if direction==1 else "SHORT",
            "entry_price": entry, "exit_price": exit_price,
            "pnl": round(actual_pnl, 2), "result": result,
            "detail": f"{exit_reason} entry=${entry:.2f} exit=${exit_price:.2f}"
        })
        ...
        push_wechat(
            f"{emoji} {trade['coin']} {direction_str} {actual_pnl:+.1f}u ({exit_reason})",
            f"Coin:{trade['coin']}\nDirection:{direction_str}\n"
            f"Entry:${entry:,.2f}\nExit:${exit_price:,.2f}\n"
            f"PnL:{actual_pnl:+.1f}u\nReason:{exit_reason}\n"
...
def run():
    ...
    if n:
        ...
        log.info("History: %d trades %.1f%% PnL%+.1f", n, w/n*100, tp)
    
    push_wechat(
        "Perp Monitor v1 Started",
        f"BTC+ETH 1H MeanReversion\nMargin:{MARGIN}u Lev:{LEVERAGE}x\n"
        f"SL:{SL_PCT}% TP:{TP_PCT}%\nHistory:{n}t PnL{tp:+.1f}u\n"
        f"{datetime.now().strftime('%H:%M')}"
    )
    log.info("Perp v1 - %s %s | %du %dx | SL=%.1f%% TP=%.1f%%",
             BAR, SYMBOLS, MARGIN, LEVERAGE, SL_PCT, TP_PCT)
    ...
    while True:
        try:
            ...
            for sym in SYMBOLS:
                ...
                if sc:
                    close = sc["close"]; rsi = sc["rsi"]; p20 = sc["p20"]
                    cid = f"{sym}_{sc['ts']}"
                    
                    if direction != 0 and cid not in SEEN:
                        # Check cooldown: don't trade same coin within 2 bars
                        if coin in LAST_SIGNAL:
                            if (datetime.now() - LAST_SIGNAL[coin]).total_seconds() < 7200:
                                status.append(f"{coin}${close:,.0f} CD")
                                continue
                        ...
                        log.info("SIGNAL %s %s @$%.2f SL=$%.2f TP=$%.2f RSI=%.0f P20=%.3f",
                                coin, dir_str, close, sl_p, tp_p, rsi, p20)
                        push_wechat(
                            f"SIGNAL {coin} {dir_str}",
                            f"Coin:{coin}\nDirection:{dir_str}\n"
                            f"Entry:${close:,.2f}\nSL:${sl_p:,.2f} TP:${tp_p:,.2f}\n"
                            f"RSI7={rsi:.0f} P20={p20:.3f} VR={sc['vr']:.2f}\n"
                            f"Margin:{MARGIN}u Notional:{NOTIONAL}u\n"
                            f"{datetime.now(timezone(timedelta(hours=8))).strftime('%Y-%m-%d %H:%M:%S')}"
                        )
                    
                    status.append(f"{coin}${close:,.0f} R{rsi:.0f} P{p20:.2f}")
                else:
                    close_v = dfs[sym]["close"].iloc[-2]
                    status.append(f"{coin}${close_v:,.0f} -")
            
            print(f"[{bj}] {' | '.join(status)} | pend:{len(PENDING)} sigs:{len(SEEN)} L{loop}")
            loop += 1
            time.sleep(POLL_SEC)
